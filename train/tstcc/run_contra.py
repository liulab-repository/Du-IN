...
from models.tstcc import tstcc_contra as tstcc_model
from models.tstcc.layers.augmentations import DataTransform
...
def init(params_):
    """
    Initialize `tstcc_contra` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.tstcc.run_contra."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    for subj_idx, subj_cfg_i in enumerate(subjs_cfg):
        ...
        ch_layouts_i = info_i.ch_layouts["mni"][ch_idxs_i,:].astype(np.float32) * 1e3
        ...
        for root_i, dirs_i, files_i in os.walk(path_dataset_i):
            for file_i in [file_i for file_i in files_i if file_i.endswith(".npy")]:
                data_items.append(utils.DotDict({
                    "subj_idx": subj_idx, "ch_idxs": ch_idxs_i, "ch_layouts": ch_layouts_i,
    ...
    params.model.tokenizer.n_channels = n_channels
    params.model.seq_len = seq_len
    params.model.tokenizer.token_len = (seq_len // (
        np.prod(params.model.tokenizer.n_strides) * np.prod(params.model.tokenizer.pool_strides)
    ))
    # Return the final `dataset_train` & `dataset_validation` & `dataset_test`.
    return dataset_train, dataset_validation, dataset_test
...
class sEEGPretrainDataset(torch.utils.data.Dataset):
    ...
    def _init_dataset(self):
        ...
        for subj_idx in range(self.n_subjects):
            ...
            # Update `n_channels` & `seq_len` according to the shape of `X_i`.
    ...
    def __getitem__(self, index):
        ...
        # Get `channel_pos` according to `date_item.ch_layouts`.
        # channel_pos - (n_channels, n_axes)
        channel_pos = data_item.ch_layouts
        ...
        X_aug1, X_aug2 = DataTransform(X[np.newaxis,...])
        X_aug1 = np.squeeze(X_aug1); X_aug2 = np.squeeze(X_aug2)
        # Construct the final data dict.
        data = {
            "X_aug1": torch.from_numpy(X_aug1.T).to(dtype=torch.float32),
            "X_aug2": torch.from_numpy(X_aug2.T).to(dtype=torch.float32),
        }
...
def train(params_):
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.tstcc.run_contra.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = tstcc_model(params.model).to(device=params.model.device)
        ...
        for epoch_idx in range(params.train.n_epochs):
            ...
            for train_batch in dataset_train:
                ...
                batch_i = [
                    train_batch["X_aug1"].to(device=params.model.device),
                    train_batch["X_aug2"].to(device=params.model.device),
                ]
                # Get the number of current `batch_i`.
                batch_size_i = len(batch_i[0].detach().cpu().numpy())
                # Train model for current batch.
                loss_i = _train(batch_i)
            ...
            for validation_batch in dataset_validation:
                ...
                batch_i = [
                    validation_batch["X_aug1"].to(device=params.model.device),
                    validation_batch["X_aug2"].to(device=params.model.device),
                ]
                # Get the number of current `batch_i`.
                batch_size_i = len(batch_i[0].detach().cpu().numpy())
                # Validate model for current batch.
                loss_i = _forward(batch_i)
                # Numpy the outputs of current batch.
                for key_i in utils.DotDict.iter_keys(loss_i):
                    utils.DotDict.iter_setattr(loss_i, key_i, utils.DotDict.iter_getattr(loss_i, key_i).detach().cpu().numpy())
                # Record information related to current batch.
            ...
            for test_batch in dataset_test:
                ...
                batch_i = [
                    test_batch["X_aug1"].to(device=params.model.device),
                    test_batch["X_aug2"].to(device=params.model.device),
                ]
                # Get the number of current `batch_i`.
                batch_size_i = len(batch_i[0].detach().cpu().numpy())
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("TS-TCC Contra for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.tstcc_params import tstcc_contra_params as tstcc_params
    ...
    tstcc_params_inst = tstcc_params(dataset=dataset)
    tstcc_params_inst.train.base = base; tstcc_params_inst.train.subjs = args.subjs
    # Train tstcc.
    train(tstcc_params_inst)
