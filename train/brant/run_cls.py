...
from models.brant import brant_cls as brant_model
...
def init(params_):
    """
    Initialize `brant_cls` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.brant.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    if len(X_s_train) == 0 or len(X_s_test) == 0: return ([], []), ([], [])
    ...
    dataset_train = FinetuneDataset(data_items=[utils.DotDict({
        "X_s": X_s_i.T, "y": y_i, "X_n": X_n_i.T,
        "seg_len": params.model.seg_len, "sfreq": load_params.resample_rate,
    }) for X_s_i, y_i, X_n_i in zip(X_s_train, y_train, X_n_train)], use_aug=True)
    dataset_validation = FinetuneDataset(data_items=[utils.DotDict({
        "X_s": X_s_i.T, "y": y_i, "X_n": X_n_i.T,
        "seg_len": params.model.seg_len, "sfreq": load_params.resample_rate,
    }) for X_s_i, y_i, X_n_i in zip(X_s_validation, y_validation, X_n_validation)], use_aug=False)
    dataset_test = FinetuneDataset(data_items=[utils.DotDict({
        "X_s": X_s_i.T, "y": y_i, "X_n": X_n_i.T,
        "seg_len": params.model.seg_len, "sfreq": load_params.resample_rate,
    }) for X_s_i, y_i, X_n_i in zip(X_s_test, y_test, X_n_test)], use_aug=False)
    ...
    n_segs = (
        params.model.seq_len // params.model.seg_len
    ); assert params.model.tokenizer.max_segs >= n_segs
    params.model.encoder.n_segs = params.model.n_segs = n_segs
    params.model.cls.d_feature = (
        params.model.encoder.n_channels *\
        params.model.encoder.n_segs *\
...
class FinetuneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_items, use_aug=False, **kwargs):
        ...
        Args:
            data_items: list - The list of data items, including [X,y,S].
    ...
    def _init_dataset(self):
        ...
        self.max_steps = self.data_items[0].X_s.shape[1] // 10
        # Initialize `seg_len` & `sfreq` from `data_items`.
        self.seg_len = self.data_items[0].seg_len; self.sfreq = self.data_items[0].sfreq
    ...
    def __getitem__(self, index):
        ...
        # X_* - (n_channels, seq_len); y - (n_labels,)
        X_s = data_item.X_s; y = data_item.y; X_n = data_item.X_n
        ## Execute data augmentations.
        if self.use_aug:
            # Randomly shift `X_*` according to `max_steps`.
            X_s_shifted = np.zeros(X_s.shape, dtype=X_s.dtype); X_n_shifted = np.zeros(X_n.shape, dtype=X_n.dtype)
            n_steps = np.random.choice((np.arange(2 * self.max_steps + 1, dtype=np.int64) - self.max_steps))
            if n_steps > 0:
                X_s_shifted[:,n_steps:] = X_s[:,:-n_steps]; X_n_shifted[:,n_steps:] = X_n[:,:-n_steps]
            elif n_steps < 0:
                X_s_shifted[:,:n_steps] = X_s[:,-n_steps:]; X_n_shifted[:,:n_steps] = X_n[:,-n_steps:]
            else:
                pass
            X_s = X_s_shifted; X_n = X_n_shifted
        ...
        # S - (n_channels, n_segs, n_bands)
        S = self._cal_power(X, sfreq=self.sfreq)
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        # Initialize the specified available_run according to subjs_cfg.
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.brant.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = brant_model(params.model)
        ...
        optimizer = torch.optim.Adam(
            ...
            params=model.parameters(), lr=params.train.lr_i,
            # Default `Adam` optimizer parameters.
            betas=(0.9, 0.999), eps=1e-8, weight_decay=0, amsgrad=False, foreach=None,
...
def _forward(inputs):
    ...
    Args:
        inputs: tuple - The input data, including [X_s,y_true,X_n].
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("Brant CLS for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.brant_params import brant_cls_params as brant_params
    ...
    brant_params_inst = brant_params(dataset=dataset); brant_params_inst.train.base = base
    brant_params_inst.train.subj = args.subj; brant_params_inst.train.pt_ckpt = args.pt_ckpt
    # Initialize the training process.
    init(brant_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train brant.
