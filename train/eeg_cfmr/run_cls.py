...
from models.eeg_cfmr import eeg_cfmr as eeg_cfmr_model
...
def init(params_):
    """
    Initialize `eeg_cfmr` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.eeg_cfmr.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    token_len = (
        seq_len // (np.prod(params.model.tokenizer.n_strides) * params.model.tokenizer.pool_stride)
    ...
    params.model.n_labels = n_labels
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.eeg_cfmr.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = eeg_cfmr_model(params.model)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("EEG-Conformer CLS for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.eeg_cfmr_params import eeg_cfmr_params as eeg_cfmr_params
    ...
    eeg_cfmr_params_inst.train.base = base; eeg_cfmr_params_inst.train.subj = args.subj
    # Initialize the training process.
    init(eeg_cfmr_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train eeg_cfmr.
