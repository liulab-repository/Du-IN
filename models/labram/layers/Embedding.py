...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, emb):
        ...
        Args:
            emb: (batch_size, *, n_segs, d_model) - The sequence of embeddings.

        Returns:
            emb: (batch_size, *, n_segs, d_model) - The sequence of time-embedded embeddings.
        """
...
class ChannelEmbedding(nn.Module):
    ...
    def __init__(self, d_model, n_channels, **kwargs):
        ...
        self.d_model = d_model; self.n_channels = n_channels
    ...
    def _init_model(self):
        ...
        Returns:
            None
        """
        # Initialize the channel embeddings.
        # channel_encodings - (n_channels, d_model)
        channel_encodings = torch.ones((self.n_channels, self.d_model), dtype=torch.float32)
        self.channel_encodings = nn.Parameter(channel_encodings, requires_grad=True)
    ...
    def _init_weight(self):
        ...
        Returns:
            None
        """
        # Initialize weights for `channel_encodings`.
        nn.init.trunc_normal_(self.channel_encodings, mean=0., std=1.)
    ...
    def forward(self, emb):
        ...
        Args:
            emb: (batch_size, *, n_channels, d_model) - The sequence of embeddings.

        Returns:
            emb: (batch_size, *, n_channels, d_model) - The sequence of channel-embedded embeddings.
        """
        # Get the channel embeddings `channel_emb`.
        # channel_emb - (n_channels, d_model)
        channel_emb = self.channel_encodings
