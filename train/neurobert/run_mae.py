...
from models.neurobert import neurobert_mae as neurobert_model
...
def init(params_):
    """
    Initialize `neurobert_mae` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.neurobert.run_mae."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    params.model.rgs.n_channels = n_channels
    params.model.seq_len = seq_len
    token_len = (
        seq_len // np.prod(params.model.tokenizer.n_strides)
    ); params.model.encoder.emb_len = params.model.tokenizer.token_len = token_len
    params.model.rgs.emb_len = params.model.decoder.token_len = token_len
    # Return the final `dataset_train` & `dataset_validation` & `dataset_test`.
    return dataset_train, dataset_validation, dataset_test
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.neurobert.run_mae.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = neurobert_model(params.model).to(device=params.model.device)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("Neuro-BERT MAE for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.neurobert_params import neurobert_mae_params as neurobert_params
    ...
    ## Initialize neurobert.
    # Initialize base.
    base = os.path.join(os.getcwd(), os.pardir, os.pardir)
    # Initialize neurobert_params.
    neurobert_params_inst = neurobert_params(dataset=dataset)
    neurobert_params_inst.train.base = base; neurobert_params_inst.train.subjs = args.subjs
    # Initialize neurobert training process, then train neurobert.
    init(neurobert_params_inst); train()
