...
from models.tstcc import tstcc_cls as tstcc_model
...
def init(params_):
    """
    Initialize `tstcc_cls` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.tstcc.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    params.model.cls.d_feature = (
        params.model.tokenizer.token_len * params.model.tokenizer.d_model
    )
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.tstcc.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = tstcc_model(params.model)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("TS-TCC CLS for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.tstcc_params import tstcc_cls_params as tstcc_params
    ...
    tstcc_params_inst = tstcc_params(dataset=dataset); tstcc_params_inst.train.base = base
    tstcc_params_inst.train.subj = args.subj; tstcc_params_inst.train.pt_ckpt = args.pt_ckpt
    # Initialize the training process.
    init(tstcc_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train tstcc_cls.
