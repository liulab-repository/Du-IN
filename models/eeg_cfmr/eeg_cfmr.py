...
class eeg_cfmr(nn.Module):
    ...
    def __init__(self, params, **kwargs):
        """
        Initialize `eeg_cfmr` object.

        Args:
            params: DotDict - Model parameters initialized by eeg_cfmr_params, updated by params.iteration.
        ...
        super(eeg_cfmr, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        """
        Forward `eeg_cfmr` to get the final predictions.
...
if __name__ == "__main__":
    ...
    from params.eeg_cfmr_params import eeg_cfmr_params
    ...
    ## Forward eeg_cfmr.
    # Instantiate params.
    eeg_cfmr_params_inst = eeg_cfmr_params(dataset=dataset)
    eeg_cfmr_params_inst.model.n_channels = n_channels
    eeg_cfmr_params_inst.model.tokenizer.n_channels = n_channels
    eeg_cfmr_params_inst.model.seq_len = seq_len
    token_len = (
        seq_len // (np.prod(eeg_cfmr_params_inst.model.tokenizer.n_strides) * eeg_cfmr_params_inst.model.tokenizer.pool_stride)
    ); eeg_cfmr_params_inst.model.encoder.emb_len = eeg_cfmr_params_inst.model.tokenizer.token_len = token_len
    eeg_cfmr_params_inst.model.cls.d_feature = (
        eeg_cfmr_params_inst.model.encoder.emb_len * eeg_cfmr_params_inst.model.encoder.d_model
    )
    eeg_cfmr_params_inst.model.n_labels = n_labels
    eeg_cfmr_params_inst.model.cls.n_labels = n_labels
    ...
    # Instantiate eeg_cfmr.
    eeg_cfmr_inst = eeg_cfmr(params=eeg_cfmr_params_inst.model); print(eeg_cfmr_inst)
    # Forward layers in `eeg_cfmr_inst`.
    # y_pred - (batch_size, n_labels); loss - DotDict
    y_pred, loss = eeg_cfmr_inst((X, y_true))
