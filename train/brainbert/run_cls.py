#!/usr/bin/env python3
"""
Created on 22:45, Mar. 25th, 2024
...
from models.brainbert import brainbert_cls as brainbert_model
...
def init(params_):
    """
    Initialize `brainbert_cls` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.brainbert.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    dataset_train = FinetuneDataset(data_items=[utils.DotDict({
        "X": X_i.T, "y": y_i, "sfreq": load_params.resample_rate,
    ...
    # Update `n_channels` & `freq_len` & `n_labels`.
    n_channels = dataset_train.n_channels; freq_len = dataset_train.freq_len; n_freqs = dataset_train.n_freqs; n_labels = len(labels)
    # Update related hyper-parameters in `params`.
    params.model.n_channels = n_channels
    params.model.cls.d_feature = (
        n_channels * params.model.encoder.d_model
    )
    params.model.freq_len = freq_len
    params.model.encoder.emb_len = params.model.tokenizer.token_len = freq_len
    params.model.tokenizer.n_freqs = params.model.n_freqs = n_freqs
...
class FinetuneDataset(torch.utils.data.Dataset):
    ...
    def _init_dataset(self):
        ...
        # Initialize `n_channels` & `freq_len` & `n_freqs` from `data_items`.
        # X - (n_channels, seq_len)
        data_item = self.data_items[0]; self.n_channels = data_item.X.shape[0]
        # S - (n_channels, freq_len, n_freqs), `freq_len = ceil(seq_len / (nperseg - noverlap)) + 1`
        S = self.stft(X=data_item.X, sfreq=data_item.sfreq)
        self.freq_len = S.shape[1]; self.n_freqs = S.shape[2]
    ...
    def stft(self, X, sfreq):
        ...
        Args:
            X: (n_channels, freq_len) - The raw time series.
    ...
    def __getitem__(self, index):
        ...
        if self.use_aug:
            ...
            if n_steps > 0:
                ...
                pass
            X = X_shifted
        ## Convert time series to spectrum series.
        # Use `stft` to get `S` from `X`.
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.brainbert.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = brainbert_model(params.model)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("BrainBERT CLS for brain signals", add_help=False)
    # Add training parmaeters.
    parser.add_argument("--seeds", type=int, nargs="+", default=[42,])
    parser.add_argument("--subj", type=str, default="033")
...
if __name__ == "__main__":
    import os
    # local dep
    from params.brainbert_params import brainbert_cls_params as brainbert_params
    ...
    brainbert_params_inst = brainbert_params(dataset=dataset); brainbert_params_inst.train.base = base
    brainbert_params_inst.train.subj = args.subj; brainbert_params_inst.train.pt_ckpt = args.pt_ckpt
    # Initialize the training process.
    init(brainbert_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train brainbert.
