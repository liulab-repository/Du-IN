...
from models.neurobert import neurobert_cls as neurobert_model
...
def init(params_):
    """
    Initialize `neurobert` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.neurobert.run_cls."
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.neurobert.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = neurobert_model(params.model)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("Neuro-BERT CLS for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.neurobert_params import neurobert_cls_params as neurobert_params
    ...
    neurobert_params_inst = neurobert_params(dataset=dataset); neurobert_params_inst.train.base = base
    neurobert_params_inst.train.subj = args.subj; neurobert_params_inst.train.pt_ckpt = args.pt_ckpt
    # Initialize the training process.
    init(neurobert_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train neurobert.
