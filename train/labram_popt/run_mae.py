...
from models.labram_popt import labram_popt_vqvae as vqkd_model
from models.labram_popt import labram_popt_mae as labram_popt_model
...
def init(params_):
    """
    Initialize `labram_popt_mae` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.labram_popt.labram_popt_mae."
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.labram_popt.run_mae.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = labram_popt_model(params.model).to(device=params.model.device)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("LaBraM-PopT MAE for brain signals", add_help=False)
    # Add training parmaeters.
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--subjs", type=str, nargs="+", default=["028",])
    parser.add_argument("--vqkd_ckpt", type=str, default=os.path.join("summaries", "2024-08-02", "2", "model", "checkpoint-50.pth"))
...
if __name__ == "__main__":
    import os
    # local dep
    from params.labram_popt_params import labram_popt_mae_params as labram_popt_params
    ...
    ## Initialize labram_popt.
    # Initialize base.
    base = os.path.join(os.getcwd(), os.pardir, os.pardir)
    # Initialize labram_popt_params.
    labram_popt_params_inst = labram_popt_params(dataset=dataset)
    labram_popt_params_inst.train.base = base; labram_popt_params_inst.train.subjs = args.subjs
    labram_popt_params_inst.train.vqkd_ckpt = args.vqkd_ckpt
    # Initialize labram_popt training process, then train labram_popt.
    init(labram_popt_params_inst); train()
