...
from models.poms.duin import duin_vqvae as vqkd_model
from models.poms.duin import duin_mae as duin_model
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.poms.duin.run_mae."
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.poms.duin.run_mae.".format(params.train.dataset))
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("Du-IN MAE for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.poms_params.duin_params import duin_mae_params as duin_params
