...
class labram_vqvae(nn.Module):
    ...
    def __init__(self, params, **kwargs):
        """
        Initialize `labram_vqvae` object.

        Args:
            params: DotDict - Model parameters initialized by labram_vqvae_params, updated by params.iteration.
        ...
        super(labram_vqvae, self).__init__(**kwargs)
    ...
    def _init_model(self):
        ...
        self.encoder = nn.Sequential(OrderedDict([
            ("emb_chan", nn.Sequential(
                ...
                ChannelEmbedding(d_model=self.params.encoder.d_model, n_channels=self.params.encoder.n_channels),
                LambdaLayer(func=(lambda x: torch.permute(x, dims=[0,2,1,3]))),
            )),
    ...
    def load_weight(self, path_ckpt):
        ...
        print((
            "INFO: Complete loading pretrained weights of modules ({}) in models.labram.labram_vqvae."
        ).format(modules))
    ...
    def forward(self, inputs):
        """
        Forward `labram_vqvae` to get the final predictions.

        Args:
            inputs: tuple - The input data, including [X,Fs].
    ...
    def quantize(self, inputs):
        """
        Forward `labram_vqvae` to get the quantized embeddings.

        Args:
            inputs: tuple - The input data, including [X_s,X_n].
...
# def labram_mae class
class labram_mae(nn.Module):
    """
    ...
    def __init__(self, params, **kwargs):
        """
        Initialize `labram_mae` object.

        Args:
            params: DotDict - Model parameters initialized by labram_mae_params, updated by params.iteration.
            kwargs: dict - The arguments related to initialize `nn.Module`-style object.
        ...
        # First call super class init function to set up `nn.Module`
        # style model and inherit it's functionality.
        super(labram_mae, self).__init__(**kwargs)
    ...
    def load_weight(self, path_ckpt):
        ...
        modules = sorted(set([key_i.split(".")[0] for key_i in model_dict.keys()]))
        print((
            "INFO: Complete loading pretrained weights of modules ({}) in models.labram.labram_mae."
        ).format(modules))
    ...
    def forward(self, inputs):
        """
        Forward `labram_mae` to get the final predictions.

        Args:
            inputs: tuple - The input data, including [X_s,c_true,X_n].
        ...
        E = [self.encoder(T_i) for T_i in T_masked]
...
# def labram_cls class
class labram_cls(nn.Module):
    """
    ...
    def __init__(self, params, **kwargs):
        """
        Initialize `labram_cls` object.

        Args:
            params: DotDict - Model parameters initialized by labram_cls_params, updated by params.iteration.
            kwargs: dict - The arguments related to initialize `nn.Module`-style object.
        ...
        # First call super class init function to set up `nn.Module`
        # style model and inherit it's functionality.
        super(labram_cls, self).__init__(**kwargs)
    ...
    def load_weight(self, path_ckpt):
        ...
        modules = sorted(set([key_i.split(".")[0] for key_i in model_dict.keys()]))
        print((
            "INFO: Complete loading pretrained weights of modules ({}) in models.labram.labram_cls."
        ).format(modules))
    ...
    def forward(self, inputs):
        """
        Forward `labram_cls` to get the final predictions.
...
if __name__ == "__main__":
    ...
    from params.labram_params import labram_vqvae_params, labram_mae_params, labram_cls_params

    # Initialize macros.
    dataset = "seeg_he2023xuanwu"; batch_size = 32
    seq_len = 800; seg_len = 200; n_segs = (seq_len // seg_len)
    n_channels = 128; n_labels = 61; n_subjects = 10

    # Initialize training process.
    utils.model.torch.set_seeds(42)

    ## Forward labram_vqvae.
    # Instantiate params.
    labram_vqvae_params_inst = labram_vqvae_params(dataset=dataset)
    labram_vqvae_params_inst.model.n_subjects = n_subjects
    labram_vqvae_params_inst.model.encoder.n_channels = labram_vqvae_params_inst.model.n_channels = n_channels
    labram_vqvae_params_inst.model.seq_len = seq_len
    labram_vqvae_params_inst.model.encoder.max_segs = labram_vqvae_params_inst.model.seq_len // labram_vqvae_params_inst.model.seg_len
    emb_len = labram_vqvae_params_inst.model.encoder.n_channels * labram_vqvae_params_inst.model.encoder.max_segs
    labram_vqvae_params_inst.model.decoder.emb_len = labram_vqvae_params_inst.model.encoder.emb_len = emb_len
    labram_vqvae_params_inst.model.rgs.n_segs = labram_vqvae_params_inst.model.seq_len // labram_vqvae_params_inst.model.seg_len
    ...
    # Instantiate labram_vqvae.
    labram_vqvae_inst = labram_vqvae(labram_vqvae_params_inst.model); print(labram_vqvae_inst)
    # Forward layers in `labram_vqvae_inst`.
    # Fs_reconstr - (2[list], batch_size, n_channels, n_segs, seg_len); loss - torch.float32
    Fs_reconstr, loss = labram_vqvae_inst((X, Fs))
    # Forward layers before vector-quantizer in `labram_vqvae_inst`.
    # E_vq - (batch_size, n_channels, n_segs, d_model); loss_vq - torch.float32
    # codex_probs - (batch_size, n_channels, n_segs, codex_size)
    E_vq, loss_vq, codex_probs = labram_vqvae_inst.quantize((X, Fs))
    ## Forward labram_mae.
    # Instantiate params.
    labram_mae_params_inst = labram_mae_params(dataset=dataset)
    labram_mae_params_inst.model.n_subjects = n_subjects
    labram_mae_params_inst.model.encoder.n_channels = labram_mae_params_inst.model.n_channels = n_channels
    labram_mae_params_inst.model.seq_len = seq_len
    labram_mae_params_inst.model.encoder.max_segs = labram_mae_params_inst.model.seq_len // labram_mae_params_inst.model.seg_len
    labram_mae_params_inst.model.encoder.emb_len = (
        labram_mae_params_inst.model.encoder.n_channels * labram_mae_params_inst.model.encoder.max_segs
    )
    # Initialize input `X` & `c_true`.
    # X - (batch_size, n_channels, n_segs, seg_len); c_true - (batch_size, n_channels, n_segs, d_model)
    codex_size = labram_mae_params_inst.model.vq.codex_size
    X = torch.rand((batch_size, n_channels, n_segs, seg_len), dtype=torch.float32)
    ...
    # Instantiate labram_mae.
    labram_mae_inst = labram_mae(labram_mae_params_inst.model); print(labram_mae_inst)
    # Forward layers in `labram_mae_inst`.
    # c_pred - (batch_size, n_channels, n_segs, d_model); loss - torch.float32
    c_pred, loss = labram_mae_inst((X, c_true))
    ## Forward labram_cls.
    # Instantiate params.
    labram_cls_params_inst = labram_cls_params(dataset=dataset)
    labram_cls_params_inst.model.n_subjects = n_subjects
    labram_cls_params_inst.model.encoder.n_channels = labram_cls_params_inst.model.n_channels = n_channels
    labram_cls_params_inst.model.seq_len = seq_len
    labram_cls_params_inst.model.encoder.max_segs = labram_cls_params_inst.model.seq_len // labram_cls_params_inst.model.seg_len
    labram_cls_params_inst.model.encoder.emb_len = (
        labram_cls_params_inst.model.encoder.n_channels * labram_cls_params_inst.model.encoder.max_segs
    )
    labram_cls_params_inst.model.cls.d_feature = (
        labram_cls_params_inst.model.encoder.emb_len * labram_cls_params_inst.model.encoder.d_model
    )
    labram_cls_params_inst.model.cls.n_labels = labram_cls_params_inst.model.n_labels = n_labels
    ...
    # Instantiate labram_cls.
    labram_cls_inst = labram_cls(labram_cls_params_inst.model); print(labram_cls_inst)
    # Forward layers in `labram_cls_inst`.
    # y_pred - (batch_size, n_labels); loss - torch.float32
    y_pred, loss = labram_cls_inst((X, y_true))
