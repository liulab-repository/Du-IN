...
from models.labram import labram_cls as labram_model
...
def init(params_):
    """
    Initialize `labram_cls` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.labram.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    dataset_train = FinetuneDataset(data_items=[utils.DotDict({
        "X": X_s_i.T, "y": y_i, "seg_len": params.model.seg_len,
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.labram.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = labram_model(params.model)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("LaBraM CLS for brain signals", add_help=False)
    # Add training parmaeters.
    parser.add_argument("--seeds", type=int, nargs="+", default=[42,])
    parser.add_argument("--subj", type=str, default="sz-021")
...
if __name__ == "__main__":
    import os
    # local dep
    from params.labram_params import labram_cls_params as labram_params
    ...
    labram_params_inst.train.base = base; labram_params_inst.train.subj = args.subj
    labram_params_inst.train.pt_ckpt = args.pt_ckpt
    # Initialize the training process.
    init(labram_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train labram.
