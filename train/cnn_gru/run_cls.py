#!/usr/bin/env python3
"""
Created on 01:41, Mar. 30th, 2024
...
from models.cnn_gru import cnn_gru as cnn_gru_model
...
def init(params_):
    """
    Initialize `cnn_gru` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.cnn_gru.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    params.model.cnn.n_channels = n_channels
    params.model.seq_len = seq_len
    token_len = (
        seq_len // np.prod(params.model.cnn.n_strides)
    ); params.model.gru.token_len = params.model.cnn.token_len = token_len
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.cnn_gru.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        model = cnn_gru_model(params.model)
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("CNN-BiGRU CLS for brain signals", add_help=False)
...
if __name__ == "__main__":
    import os
    # local dep
    from params.cnn_gru_params import cnn_gru_params as cnn_gru_params
    ...
    cnn_gru_params_inst.train.base = base; cnn_gru_params_inst.train.subj = args.subj
    # Initialize the training process.
    init(cnn_gru_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train cnn_gru.
