...
from models.labram_popt import labram_popt_cls as labram_popt_model
...
def init(params_):
    """
    Initialize `labram_popt_cls` training variables.
...
def load_data(load_params):
    ...
    try:
        ...
        raise ValueError((
            "ERROR: Unknown dataset type {} in train.labram_popt.run_cls."
...
def _load_data_seeg_he2023xuanwu(load_params):
    ...
    # Initialize `n_subjects` & `n_subjects` & `seq_len` & `n_labels` from `load_params`.
    n_channels = None; n_subjects = 1; seq_len = None; n_labels = None
    ...
    X_s = dataset.X_s.astype(np.float32); y = dataset.y.astype(np.int64); X_n = dataset.X_n.astype(np.float32)
    assert X_s.shape == X_n.shape
    ...
    if load_params.type.startswith("bipolar"):
        ...
        sample_rate = 1000
        X_s = sp.signal.resample(X_s, int(np.round(X_s.shape[1] / (sample_rate / load_params.resample_rate))), axis=1)
        X_n = sp.signal.resample(X_n, int(np.round(X_n.shape[1] / (sample_rate / load_params.resample_rate))), axis=1)
        # Truncate data according to epoch range (-0.2,1.0), the original epoch range is (-0.5,2.0).
        X_s = X_s[:,int(np.round((-0.5 - (-0.5)) * load_params.resample_rate)):\
            int(np.round((2.5 - (-0.5)) * load_params.resample_rate)),:]
        X_n = X_n[:,int(np.round((-0.5 - (-0.5)) * load_params.resample_rate)):\
            int(np.round((2.5 - (-0.5)) * load_params.resample_rate)),:]
        ...
        X_s = (X_s - np.mean(X_s, axis=(0,1), keepdims=True)) / np.std(X_s, axis=(0,1), keepdims=True)
        X_n = (X_n - np.mean(X_n, axis=(0,1), keepdims=True)) / np.std(X_n, axis=(0,1), keepdims=True)
    ...
    X_s_train = X_s[train_idxs,:,:]; y_train = y[train_idxs]; X_n_train = X_n[train_idxs,:,:]
    X_s_test = X_s[test_idxs,:,:]; y_test = y[test_idxs]; X_n_test = X_n[test_idxs,:,:]
    # Check whether trainset & testset both have data items.
    if len(X_s_train) == 0 or len(X_s_test) == 0: return None, None, None
    # Make sure there is no overlap between X_*_train & X_*_test.
    samples_same = None; n_samples = 10; assert X_s_train.shape[1] == X_s_test.shape[1]
    for _ in range(n_samples):
        sample_idx = np.random.randint(X_s_train.shape[1])
        sample_same_i = np.intersect1d(X_s_train[:,sample_idx,0], X_s_test[:,sample_idx,0], return_indices=True)[-1].tolist()
    ...
    validation_idxs = np.random.choice(np.arange(X_s_test.shape[0]), size=int(X_s_test.shape[0]/2), replace=False)
    validation_mask = np.zeros((X_s_test.shape[0],), dtype=np.bool_); validation_mask[validation_idxs] = True
    X_s_validation = X_s_test[validation_mask,:,:]; y_validation = y_test[validation_mask,:]
    X_n_validation = X_n_test[validation_mask,:,:]
    X_s_test = X_s_test[~validation_mask,:,:]; y_test = y_test[~validation_mask,:]; X_n_test = X_n_test[~validation_mask,:,:]
    # Log information of data loading.
    msg = (
        "INFO: Data preparation for subject ({}) complete, with train-set ({}) & validation-set ({}) & test-set ({})."
    ).format(subj_cfg.name, X_s_train.shape, X_s_validation.shape, X_s_test.shape)
    print(msg); paths.run.logger.summaries.info(msg)
    # Update `n_channels` & `seq_len` & `n_labels`.
    n_channels = X_s.shape[-1]; seq_len = X_s.shape[-2]; n_labels = len(labels)
    # Shuffle dataset to fuse different subjects.
    train_idxs = np.arange(X_s_train.shape[0]); np.random.shuffle(train_idxs)
    validation_idxs = np.arange(X_s_validation.shape[0]); np.random.shuffle(validation_idxs)
    test_idxs = np.arange(X_s_test.shape[0]); np.random.shuffle(test_idxs)
    X_s_train = X_s_train[train_idxs,...]; y_train = y_train[train_idxs,...]; X_n_train = X_n_train[train_idxs,...]
    X_s_validation = X_s_validation[validation_idxs,...]; y_validation = y_validation[validation_idxs,...]
    X_n_validation = X_n_validation[validation_idxs,...]
    X_s_test = X_s_test[test_idxs,...]; y_test = y_test[test_idxs,...]; X_n_test = X_n_test[test_idxs,...]
    # Log information of data loading.
    msg = (
        "INFO: Data preparation complete, with train-set ({}) & validation-set ({}) & test-set ({})."
    ).format(X_s_train.shape, X_s_validation.shape, X_s_test.shape)
    print(msg); paths.run.logger.summaries.info(msg)
    # TODO: Since we don't load MNI coordinates from behavior file, we initialize random MNI coordinates for each channel.
    # channel_pos - (n_channels, n_axes)
    channel_pos = np.random.uniform(low=-1., high=1., size=(n_channels, params.model.n_axes)) * 100.
    # Construct dataset from data items.
    dataset_train = FinetuneDataset(data_items=[utils.DotDict({
        "X": X_s_i.T, "y": y_i, "seg_len": params.model.seg_len, "channel_pos": channel_pos,
    }) for X_s_i, y_i in zip(X_s_train, y_train)], use_aug=True)
    dataset_validation = FinetuneDataset(data_items=[utils.DotDict({
        "X": X_s_i.T, "y": y_i, "seg_len": params.model.seg_len, "channel_pos": channel_pos,
    }) for X_s_i, y_i in zip(X_s_validation, y_validation)], use_aug=False)
    dataset_test = FinetuneDataset(data_items=[utils.DotDict({
        "X": X_s_i.T, "y": y_i, "seg_len": params.model.seg_len, "channel_pos": channel_pos,
    }) for X_s_i, y_i in zip(X_s_test, y_test)], use_aug=False)
    ...
    # TODO: We don't update max_segs of model.encoder parameters.
    params.model.n_subjects = n_subjects
    params.model.encoder.n_channels = params.model.n_channels = n_channels
    params.model.seq_len = seq_len
    n_segs = params.model.seq_len // params.model.seg_len
    params.model.encoder.emb_len = (
        params.model.encoder.n_channels * n_segs
    )
...
class FinetuneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_items, use_aug=False, **kwargs):
        ...
        Args:
            data_items: list - The list of data items, including [X,y,channel_pos].
    ...
    def _init_dataset(self):
        ...
        # Initialize the length of segment.
        self.seg_len = self.data_items[0].seg_len
    ...
    def __getitem__(self, index):
        ...
        # X - (n_channels, seq_len); y - (n_labels,); channel_pos - (n_channels, n_axes)
        X = data_item.X; y = data_item.y; channel_pos = data_item.channel_pos
        ...
        data = {
            ...
            "channel_pos": torch.from_numpy(channel_pos).to(dtype=torch.float32),
        }
...
def train():
    ...
    if params.train.dataset == "seeg_he2023xuanwu":
        ...
        # Initialize the subject configuration according to `params.train.subj`.
        ...
        raise ValueError("ERROR: Unknown dataset {} in train.labram_popt.run_cls.".format(params.train.dataset))
    ...
    for load_params_idx in range(len(load_params)):
        ...
        ).format(load_params_i.name, subj_cfg.name)
        print(msg); paths.run.logger.summaries.info(msg)
        ...
        model = labram_popt_model(params.model)
        ...
        for epoch_idx in range(params.train.n_epochs):
            ...
            for train_batch in dataset_train:
                ...
                batch_i = [
                    ...
                    train_batch["channel_pos"].to(device=params.model.device),
                ]
            ...
            for validation_batch in dataset_validation:
                ...
                batch_i = [
                    ...
                    validation_batch["channel_pos"].to(device=params.model.device),
                ]
                # Get the number of current `batch_i`.
                batch_size_i = len(batch_i[0].detach().cpu().numpy())
            ...
            for test_batch in dataset_test:
                ...
                batch_i = [
                    ...
                    test_batch["channel_pos"].to(device=params.model.device),
                ]
                # Get the number of current `batch_i`.
                batch_size_i = len(batch_i[0].detach().cpu().numpy())
...
def get_args_parser():
    ...
    """
    # Initialize parser.
    parser = argparse.ArgumentParser("LaBraM-PopT CLS for brain signals", add_help=False)
    # Add training parmaeters.
    parser.add_argument("--seeds", type=int, nargs="+", default=[42,])
    parser.add_argument("--subj", type=str, default="028")
...
if __name__ == "__main__":
    import os
    # local dep
    from params.labram_popt_params import labram_popt_cls_params as labram_popt_params
    ...
    # Initialize labram_popt_params.
    labram_popt_params_inst = labram_popt_params(dataset=dataset)
    labram_popt_params_inst.train.base = base; labram_popt_params_inst.train.subj = args.subj
    labram_popt_params_inst.train.pt_ckpt = args.pt_ckpt
    # Initialize the training process.
    init(labram_popt_params_inst)
    # Loop the training process over random seeds.
    for seed_i in args.seeds:
        # Initialize random seed, then train labram_popt.
